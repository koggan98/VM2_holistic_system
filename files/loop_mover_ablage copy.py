...
public:
    ...
    : Node("moveit_mover") {
        ...
        RCLCPP_INFO(this->get_logger(), "Loop Mover Node initialized.");
    }
...
private:
    ...
    void handPositionCallback(const geometry_msgs::msg::Pose::SharedPtr msg) {
        if (!tool_selected_) {
            RCLCPP_INFO(this->get_logger(), "Ignoring hand pose – no tool selected.");
            return;
        }
    ...
    void toolSelectionCallback(const std_msgs::msg::String::SharedPtr msg) {
        if (!handover_completed_) {
            RCLCPP_WARN(this->get_logger(), "Previous handover not completed, ignoring new tool selection.");
            return;
        }
    ...
    void performHandoverToHandPose() {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(1000));
            handover_completed_ = true;
            tool_selected_ = false;
        } else {
