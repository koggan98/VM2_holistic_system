...
private:
    ...
    void performHandoverToHandPose() {
        // Fahre über die Hand
        geometry_msgs::msg::Pose above_pose = last_hand_pose_;
        above_pose.position.x -= 0.2;
        above_pose.position.z += 0.25;
        ...
        // Absenken zur Hand
        geometry_msgs::msg::Pose final_pose = above_pose;
        final_pose.position.z = last_hand_pose_.position.z + 0.15;
    
        move_group_->setPoseTarget(final_pose);
        if (move_group_->plan(plan) == moveit::core::MoveItErrorCode::SUCCESS) {
            RCLCPP_INFO(this->get_logger(), "Lowering to hand...");
            move_group_->execute(plan);
        } else {
            RCLCPP_ERROR(this->get_logger(), "Planning to hand failed.");
            return;
        }
